...
data class AggregateComputations(val tokenComputations: MutableMap<Int, MutableList<TextModification>>, val textComputations: MutableMap<Int, MutableList<TextModification>>)
class FastTokenStreamRewriter(val tokenStream: TokenStream) {
    ...
    fun insertAfter(t: Token?, text: Any, insertionType: InsertionType) {
        if(t==null) return
        tokenComputations.getOrPut(t.tokenIndex) { mutableListOf() }.add(TextModification(text.toString(), Order.AFTER, insertionType))
        textComputations.getOrPut(t.stopIndex) { mutableListOf() }.add(TextModification(text.toString(), Order.AFTER, insertionType))
    }

    fun insertBefore(t: Token?, text: Any, insertionType: InsertionType) {
        if(t==null) return
        tokenComputations.getOrPut(t.tokenIndex) { mutableListOf() }.add(TextModification(text.toString(), Order.BEFORE, insertionType))
        textComputations.getOrPut(t.startIndex) { mutableListOf() }.add(TextModification(text.toString(), Order.BEFORE, insertionType))

    }

    fun replace(t: Token?, text: Any) {
        if(t==null) return
        tokenComputations.getOrPut(t.tokenIndex) { mutableListOf() }.add(TextModification(text.toString(), Order.REPLACE))
        textComputations.getOrPut(t.startIndex) { mutableListOf() }.add(TextModification(text.toString(), Order.REPLACE, range = t.startIndex..t.stopIndex))
    }

    fun replace(from: Token?, to: Token?, text: Any) {
        if(from==null || to==null) return
        tokenComputations.getOrPut(from.tokenIndex) { mutableListOf() }.add(TextModification(text.toString(), Order.REPLACE, range = from.tokenIndex..to.tokenIndex))
        textComputations.getOrPut(from.startIndex) { mutableListOf() }.add(TextModification(text.toString(), Order.REPLACE, range = from.startIndex..to.stopIndex))
    }

    fun delete(t: Token?) {
        if(t==null) return
        tokenComputations.getOrPut(t.tokenIndex) { mutableListOf() }.add(TextModification("", Order.DELETE))
        textComputations.getOrPut(t.startIndex) { mutableListOf() }.add(TextModification("", Order.DELETE, range = t.startIndex..t.stopIndex))
    }

    fun delete(from: Token?, to: Token?) {
        if(from==null || to==null) return
        tokenComputations.getOrPut(from.tokenIndex) { mutableListOf() }.add(TextModification("", Order.DELETE, range = from.tokenIndex..to.tokenIndex))
        textComputations.getOrPut(from.startIndex) { mutableListOf() }.add(TextModification("", Order.DELETE, range = from.startIndex..to.stopIndex))
    }
